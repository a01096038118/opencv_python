...
print(f'np_arr: {np_arr}')              # list type
print(f'np_arr type: {type(np_arr)}')   # numpy.ndarray type
...
for idx, num in enumerate(np_arr):
    print(f'idx: {idx}')
    for arr in np_arr:
        for num in arr:
            print(f'num: {num}')  # 전체 데이터 조회
...
np_arr = np_arr.astype(np.int64)            # int64: 정수라고 생각하면 됨
print(f'np_arr: {np_arr}')                  # [1 2 3] 실수 --> 정수
print(f'np_arr type: {type(np_arr)}')       # numpy.ndarray type
...
# 배열 차원 확인
print(f'차원: {np_arr.ndim}')               # 2차원

# 배열 형태 확인
print(f'형태: {np_arr.shape}')              # (3, 5) 3행 5열

# 배열 데이터 타입
print(f'데이터 타입: {np_arr.dtype}')        # 데이터 타입: float64 
                                            # 정수와 float가 만나면 float가 데이터를 통일시킴
